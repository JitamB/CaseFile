...
| `data/corpus/authored/` — the ~25 signal/misdirection documents | Reviewable as a diff; the ~46k-document noise floor is templates, regenerated byte-identically by `make data` instead — see [§24](03-data.md), corrected at ladder step 1.5 |
...
`main`, as three jobs — the third joined at ladder step 1.9:
...
| **ui** | `npm ci` · `npm test` · `npm run build`, against `fixtures/` at the repo root | A screen that renders correctly today and silently stops matching a fixture tomorrow |
...
def test_scenario_d_closes_at_verify_with_no_model_calls():
    case = run_pipeline("net_revenue", "West", "2026-03")
...
| 1.6 | `retrieval/` — footprint filter → BM25 (embeddings behind an extra) | Jitam | 64.6k → ~1.0–1.3k → top 15 on the East fixture |
...
**Two rows executed out of the printed order, for the same reason 0.4 moved ahead of 0.2 at
P0: a step whose verify command depends on an artifact a later step creates would merge green
having verified nothing.**

- **1.3 and 1.4 needed §27 items 8 and 9 first.** 1.3's verify names scenarios D and E, and
  1.4 reads `composition` edges naming KPIs that had no contract yet. The remaining five
  contracts and scenarios D/E were built as their own PRs immediately before 1.3, and 1.3/1.4
  proceeded as printed once they existed.
- **1.8 was taken before 1.7.** Entitlement's security test needs personas that exist to run
  against; §44's own opening line says steps are ordered by dependency, and the persona specs
  are 1.7's dependency, not its successor.
