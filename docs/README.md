...
> **These nine files are the canonical project plan — the single source of truth.**
> Earlier working drafts are kept locally in `initials/`, outside the repository; where
> they disagree with `docs/`, `docs/` wins.
...
An `initials/` directory of earlier drafts sits alongside this one in each member's
checkout. It is **not part of the repository** — everything in it that still matters has
been carried into these nine files, and where the two disagree, `docs/` wins.
